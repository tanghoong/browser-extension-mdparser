...
window.PresentationManager = {
  ...
  async createSlideElements() {
    ...
    this.slideMarkdown.forEach((slideMarkdown, index) => {
      ...
      // Note: marked.js sanitizes HTML output by default to prevent XSS attacks.
  ...
  toggleNotes() {
    ...
    // Look for notes in current slide (blockquote with [!NOTE] or speaker notes marked with :::notes)
    let notesPanel = document.querySelector('.slide-notes-panel');
    ...
    // Find notes content in slide
    const noteContent = this.extractNotes(currentSlide);
    
    if (!noteContent) {
      console.log('No notes found in this slide');
      return;
    }
    
    // Create notes panel
    notesPanel = document.createElement('div');
    notesPanel.className = 'slide-notes-panel visible';
    notesPanel.innerHTML = `
      <div class="notes-header">
        <span>Speaker Notes</span>
        <button class="notes-close" title="Close (N)">&times;</button>
      </div>
  ...
  extractNotes(slide) {
    // Look for notes marked as blockquote with [!NOTE] or special class
    const noteBlocks = slide.querySelectorAll('blockquote');
    let notes = '';
    
    noteBlocks.forEach(block => {
      const text = block.innerHTML;
      if (text.includes('[!NOTE]') || text.includes('[!SPEAKER]') || block.classList.contains('speaker-notes')) {
        notes += text.replace('[!NOTE]', '').replace('[!SPEAKER]', '');
      }
    });
    
    // Also check for hidden notes div
    const hiddenNotes = slide.querySelector('.slide-notes, .speaker-notes');
    if (hiddenNotes) {
      notes += hiddenNotes.innerHTML;
    }
